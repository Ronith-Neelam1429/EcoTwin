...
The effect lenses compare every edited cell with that same cell in the baseline. **Cooling** shows the reduction in equilibrium surface temperature, **Sun & shade** combines modeled canopy shade with the reduction in absorbed shortwave solar radiation, and **Stormwater** shows event runoff depth avoided. Neutral cells are unchanged; stronger color means a larger modeled improvement. These are spatial comparison layers, not UV measurements, pedestrian shade footprints, flood depths, or routed downstream effects. Selecting a cell exposes the numerical differences behind its color.
