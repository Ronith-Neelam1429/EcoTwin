...
export function MetricsPanel({
  ...
  const runoffDelta =
    ((current.totalRunoff - baseline.totalRunoff) / baseline.totalRunoff) * 100;
  ...
  const metrics = [
    ...
    {
      label: "Total runoff",
      value: current.totalRunoff.toLocaleString(undefined, {
  ...
  return (
    <aside className="metrics-panel">
      ...
      <p className="model-note">
        Illustrative estimates on a 10m grid. Unmapped ground uses assumed
        properties; these are not measured site conditions. Runoff is in model
        units.
      </p>
      ...
      <div className="impact-block">
        ...
        <div className="impact-row">
          <span>Runoff</span>
          <strong className={runoffDelta <= 0 ? "positive" : "negative"}>
            {signed(runoffDelta, "%")}
          </strong>
        </div>
        <div className="impact-row">
