...
export function EcoTwinView({ location }: { location: TwinLocation }) {
  ...
  useEffect(() => {
    let active = true;
    loadNeighborhood(location).then((data) => {
      if (active) setResult(data);
    }).catch((reason: unknown) => {
      if (active) setError(reason instanceof Error ? reason.message : "Map data is unavailable. Please retry.");
    });
    return () => { active = false; };
  }, [location, attempt]);
  ...
  if (!result) return (
    <div className="geography-loading" role="status">
      <MapPin size={28} />
      <h2>{error ? "Neighborhood data unavailable" : "Loading your neighborhood"}</h2>
      <p>{error || "Finding real building outlines, roads, and mapped green areas within 150 metres of your location."}</p>
      {error ? <button type="button" onClick={() => { setError(""); setAttempt((n) => n + 1); }}>Retry map data</button> : <span className="loading-track" />}
      <small>OpenStreetMap · Geometry only; no Street View images are reconstructed.</small>
    </div>
  );
  return <LoadedTwin location={location} neighborhood={result} />;
}

function LoadedTwin({ location, neighborhood }: { location: TwinLocation; neighborhood: Neighborhood }) {
  ...
  function updateCell(id: string) {
    ...
    if (cell.buildingId && selectedTool !== "green_roof" && selectedTool !== "erase") {
      setEditMessage("This cell contains a building. Use Green Roof here, or select open ground.");
      return;
    }
    setEditMessage("");
    ...
    <section className="twin-workspace">
      <div className="twin-toolbar">
        <div className="twin-location">
          ...
          <div>
            <span className="panel-kicker">Mapped neighborhood · 300m × 300m</span>
      ...
      <div className="twin-layout">
        ...
        <div className="geographic-scene">
          <EcoTwinScene cells={cells} viewMode={viewMode} onCellClick={updateCell} neighborhood={neighborhood} location={location} />
          <div className="geography-summary">
            <strong>{neighborhood.buildings} mapped buildings · {neighborhood.roads} road segments</strong>
            <span>{neighborhood.assumedHeights} heights estimated · Road widths approximate · Flat terrain</span>
            <span>{Math.round(neighborhood.unknownCells / 900 * 100)}% of grid has unmapped ground cover</span>
            {neighborhood.buildings === 0 && <span>No building footprints are available here in OpenStreetMap.</span>}
          </div>
          {editMessage && <div className="edit-message" role="status">{editMessage}</div>}
        </div>
